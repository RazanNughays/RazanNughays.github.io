---
layout: single
title: "Research Interests"
permalink: /research/
author_profile: false
---

<style>
  .page__content {
    max-width: 1400px;
    margin: 0 auto;
  }

  .page__title {
    text-align: left !important;
    max-width: 1400px;
    margin: 0 auto 1.2rem auto;
  }

  .research-wrap {
    width: 100%;
  }

  .research-center {
    text-align: center;
  }

  .research-lead {
    font-size: 1.15rem;
    line-height: 1.8;
    margin-bottom: 1.5rem;
  }

  .research-question {
    text-align: center;
    font-size: 2rem;
    font-weight: 700;
    line-height: 1.35;
    margin: 1rem 0 1.75rem 0;
  }

  .research-caption {
    text-align: center;
    font-size: 0.95rem;
    line-height: 1.6;
    margin-bottom: 2rem;
  }

  .research-section-title {
    margin-top: 3.5rem;
    margin-bottom: 1.25rem;
    padding-bottom: 0.5rem;
    border-bottom: 1px solid var(--global-border-color);
    font-size: 2rem;
    font-weight: 700;
  }

  .research-row {
    display: flex;
    gap: 72px;
    align-items: flex-start;
    margin: 3.5rem 0;
    width: 100%;
  }

  .research-text {
    flex: 1.65;
    min-width: 0;
  }

  .research-media {
    flex: 1;
    min-width: 0;
    text-align: center;
  }

  .research-item-title {
    font-size: 1.5rem;
    font-weight: 700;
    line-height: 1.35;
    margin-bottom: 1rem;
  }

  .research-text p {
    font-size: 1.05rem;
    line-height: 1.8;
  }

  .research-link {
    margin-top: 1rem;
  }

  .research-link a {
    font-weight: 700;
  }

  .research-media img,
  .research-media video {
    width: 100%;
    max-width: 500px;
    height: auto;
    border-radius: 10px;
    display: block;
    margin: 0 auto;
  }

  .research-bottom-img {
    width: 100%;
    max-width: 560px;
    height: auto;
    display: block;
    margin: 0 auto 1rem auto;
  }

  .research-subsection-title {
    margin-top: 3rem;
    margin-bottom: 1rem;
    font-size: 1.5rem;
    font-weight: 700;
  }

  .research-bottom-text {
    font-size: 1.05rem;
    line-height: 1.8;
  }

  @media (max-width: 900px) {
    .page__content,
    .page__title {
      max-width: 100%;
    }

    .research-question {
      font-size: 1.6rem;
    }

    .research-row {
      flex-direction: column;
      gap: 28px;
      margin: 2.75rem 0;
    }

    .research-text,
    .research-media {
      text-align: center;
    }

    .research-media img,
    .research-media video {
      max-width: 100%;
    }
  }
</style>

<div class="research-wrap">

  <h3>Focus Areas</h3>
  <ul>
    <li>Nonlinear Optical Microscopy and Spectroscopy</li>
    <li>Ultrafast Charge-Carrier Dynamics</li>
    <li>Light–Matter Interactions</li>
    <li>Interfacial Energy Transport</li>
  </ul>

  <hr>

  <p class="research-lead">My research is driven by a deep curiosity:</p>

  <div class="research-question">
    Why do materials that appear similar often exhibit dramatically different performance?
  </div>

  <p class="research-lead">
    In functional materials, energy flow is controlled by surface structure (crystallographic orientation, termination, and composition), symmetry, and interfacial electronic properties. My research seeks to understand how these factors shape carrier dynamics and influence material performance, ultimately guiding the rational design of next-generation functional materials and devices.
  </p>

  <h2 class="research-section-title">Orientation and Termination Effects on Charge Carrier Dynamics</h2>

  <div class="research-row">
    <div class="research-text">
      <div class="research-item-title">Perovskites for Solar and Sensing Applications</div>

      <p>
        Using ultrafast scanning electron microscopy (USEM), we investigated how crystallographic orientation and surface termination govern charge-carrier dynamics in MAPbI₃ perovskite single crystals. The results reveal strong facet-dependent transport: the (001) surface supports long-range carrier diffusion, while the (100) facet introduces defect states that accelerate recombination.
      </p>

      <p class="research-link">
        <a href="https://doi.org/10.1002/adom.202300190" target="_blank" rel="noopener"><strong>Read full paper →</strong></a>
      </p>
    </div>

    <div class="research-media">
      <video autoplay loop muted playsinline>
        <source src="/images/dif.mp4" type="video/mp4">
      </video>
    </div>
  </div>

  <div class="research-row">
    <div class="research-text">
      <div class="research-item-title">CdTe X-ray Detectors</div>

      <p>
        Photoinduced carrier dynamics in CdTe single crystals were investigated to understand the role of crystallographic orientation in X-ray detector performance. Devices fabricated from the (110) facet exhibit more than two orders of magnitude lower detection limits and significantly higher sensitivity compared to (100) and (111) orientations.
      </p>

      <p class="research-link">
        <a href="https://doi.org/10.1016/j.xcrp.2023.101723" target="_blank" rel="noopener"><strong>Read full paper →</strong></a>
      </p>
    </div>

    <div class="research-media">
      <img src="/images/Xray.png"
           alt="Orientation-dependent X-ray detector performance in CdTe">
    </div>
  </div>

  <h2 class="research-section-title">Composition-Dependent Carrier Dynamics</h2>

  <div class="research-row">
    <div class="research-text">
      <div class="research-item-title">Mixed-Cation Perovskites</div>

      <p>
        Using four-dimensional ultrafast scanning electron microscopy (4D-USEM), we investigated surface carrier dynamics in mixed-cation perovskites FA₀.₆MA₀.₄PbI₃ and FA₀.₄MA₀.₆PbI₃. The measurements reveal composition-dependent carrier lifetimes governed by surface defects and ion migration. Higher FA content promotes iodide migration that passivates surface vacancies, leading to fewer defects and longer-lived charge carriers.
      </p>

      <p class="research-link">
        <a href="https://doi.org/10.1002/advs.202404468" target="_blank" rel="noopener"><strong>Read full paper →</strong></a>
      </p>
    </div>

    <div class="research-media">
      <video autoplay loop muted playsinline>
        <source src="/images/mig.mp4" type="video/mp4">
      </video>
    </div>
  </div>

  <div class="research-row">
    <div class="research-text">
      <div class="research-item-title">Te–Se Nanocomposites</div>

      <p>
        Surface carrier dynamics in Te–Se nanocomposites reveal strong composition-dependent surface behavior: TeSe (1:0.5) exhibits long-lived carrier diffusion, while TeSe (1:1) shows localized carriers and faster recombination.
      </p>

      <p class="research-link">
        <a href="https://doi.org/10.1021/acs.jpcc.5c07387" target="_blank" rel="noopener"><strong>Read full paper →</strong></a>
      </p>
    </div>

    <div class="research-media">
      <img src="/images/TeSe.png"
           alt="Composition-dependent surface dynamics in Te–Se nanocomposites">
    </div>
  </div>

  <h3 class="research-subsection-title">Instrumentation Development</h3>

  <p class="research-bottom-text">
    I am particularly interested in advancing ultrafast microscopy approaches including <strong>4D-USEM</strong> and <strong>time-resolved second harmonic generation (TR-SHG)</strong> that combine nanometer spatial resolution with femtosecond temporal precision to correlate nanoscale structure with dynamic function.
  </p>

  <p class="research-bottom-text">
    My goal is to establish quantitative structure–dynamics–function relationships that connect nanoscale morphology to macroscopic device performance. By bridging real-space imaging with nonlinear spectroscopy, I aim to enable predictive design principles for next-generation photovoltaic, sensing, and energy-conversion technologies.
  </p>

  <h3 class="research-subsection-title">Foundations in Ultrafast Electron Microscopy</h3>

  <p class="research-center">
    <img src="/images/4D-USEM.png"
         alt="4D-USEM system"
         class="research-bottom-img">
  </p>

  <p class="research-bottom-text">
    This direction builds on my doctoral research, where I employed and refined 4D-USEM to directly visualize charge-carrier dynamics in real space and real time.
  </p>

</div>
